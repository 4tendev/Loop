...
export function AvalonTableSeat(props: AvalonTableSeatProps) {
  ...
  const isSpeaking = Boolean(
    seat.player && props.speakingUserIds.includes(seat.player.id),
  );
  ...
  const speakingBadge = isSpeaking ? (
    <span
      aria-label={`${seat.player?.name}: در حال صحبت`}
      className="pointer-events-none absolute -right-3 -top-2 z-[60] rounded-full border border-success-content/25 bg-success px-1.5 py-0.5 text-[0.5rem] font-black leading-none text-success-content shadow-lg motion-safe:animate-pulse"
      title="در حال صحبت"
    >
      🎙 صحبت
    </span>
  ...
  if (props.variant === "grid") {
    ...
    return <div className="relative z-10 flex min-h-12 min-w-12 flex-1 items-center justify-center"><span className={`absolute bg-base-300 ${connectorClasses[position]}`} /><button aria-label={seat.player ? `صندلی ${seat.number}: ${seat.player.name} (${presenceLabel}${isSpeaking ? "، در حال صحبت" : ""})` : `صندلی ${seat.number}`} className={`relative z-10 flex items-center justify-center rounded-full border-2 text-sm font-semibold shadow-sm transition ${isWinningSeat ? "avalon-winner-seat h-16 w-16" : "h-12 w-12"} ${tone} ${isOwnSeat ? "outline outline-2 outline-offset-2 outline-primary shadow-primary/40" : ""} ${isSpeaking ? "ring-4 ring-success/60 shadow-lg shadow-success/50" : ""}`} data-winner-side={isWinningSeat ? game.winnerSide : undefined} onClick={handleClick} title={seat.player ? `${seat.player.name} · ${presenceLabel}` : "صندلی خالی"} type="button">{isWinningSeat ? <span aria-hidden="true" className="avalon-winner-sparkles">✦</span> : null}{sleepingBadge}{decisionBadge}{presenceBadge}{speakingBadge}{isOwnSeat ? <span className="pointer-events-none absolute -right-3 top-1/2 z-20 -translate-y-1/2 rounded-full border border-primary/50 bg-primary px-1.5 py-0.5 text-[0.55rem] font-black uppercase leading-none text-primary-content shadow">شما</span> : null}{isLadySeat ? <img alt="Lady of the Lake" className="pointer-events-none absolute -left-2 bottom-0 z-30 h-7 w-7 translate-y-1/2 rounded-full border border-secondary/50 bg-base-100 object-cover p-0.5 shadow-lg" src="/avalon/avalon_characters/LadyOfTheLake.png" title={`Lady of the Lake · Seat ${seat.number}`} /> : null}{isMateSeat ? <img alt="Selected teammate" className="pointer-events-none absolute -right-2 bottom-0 z-20 h-7 w-7 translate-y-1/2 rounded-full border border-success/50 bg-base-100 object-cover p-0.5 shadow-lg" src="/avalon/Mate.png" title={`Seat ${seat.number}`} /> : null}{seat.player ? <img alt={seatImageAlt} className={`relative z-10 rounded-full object-cover transition ${isWinningSeat ? "h-14 w-14" : "h-10 w-10"} ${isOnline || props.isTerminalGame ? "" : "grayscale opacity-55"}`} src={seatImage} /> : seat.number}</button></div>;
  }
