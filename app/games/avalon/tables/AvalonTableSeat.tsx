...
export function AvalonTableSeat(props: AvalonTableSeatProps) {
  ...
  if (props.variant === "grid") {
    ...
    return <div className="relative z-10 flex min-h-12 min-w-12 flex-1 items-center justify-center"><span className={`absolute bg-base-300 ${connectorClasses[position]}`} /><button aria-label={seat.player ? `صندلی ${seat.number}: ${seat.player.name} (${presenceLabel})` : `صندلی ${seat.number}`} className={`relative z-10 flex h-12 w-12 items-center justify-center rounded-full border-2 text-sm font-semibold shadow-sm transition ${tone} ${isOwnSeat ? "outline outline-2 outline-offset-2 outline-primary shadow-primary/40" : ""}`} onClick={handleClick} title={seat.player ? `${seat.player.name} · ${presenceLabel}` : "صندلی خالی"} type="button">{sleepingBadge}{decisionBadge}{presenceBadge}{isOwnSeat ? <span className="pointer-events-none absolute -right-3 top-1/2 z-20 -translate-y-1/2 rounded-full border border-primary/50 bg-primary px-1.5 py-0.5 text-[0.55rem] font-black uppercase leading-none text-primary-content shadow">شما</span> : null}{isMateSeat ? <img alt="Selected teammate" className="pointer-events-none absolute -right-2 bottom-0 z-20 h-7 w-7 translate-y-1/2 rounded-full border border-success/50 bg-base-100 object-cover p-0.5 shadow-lg" src="/avalon/Mate.png" title={`Seat ${seat.number}`} /> : null}{seat.player ? <img alt={seatImageAlt} className={`h-10 w-10 rounded-full object-cover transition ${isOnline ? "" : "grayscale opacity-55"}`} src={seatImage} /> : seat.number}</button></div>;
  }
  ...
  return <button aria-label={seat.player ? `صندلی ${seat.number}: ${seat.player.name} (${presenceLabel})` : `صندلی ${seat.number}`} className={`absolute z-20 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-2 text-xs font-bold shadow-lg transition sm:h-14 sm:w-14 ${tone} ${canToggleTeamSeat || canSelectLadyTarget || canSelectAssassinTarget ? "ring-2 ring-warning/70" : ""} ${isOwnSeat ? "outline outline-2 outline-offset-2 outline-primary shadow-primary/40" : ""}`} onClick={handleClick} title={seat.player ? `${seat.player.name} · ${presenceLabel}` : "صندلی خالی"} type="button" style={seatStyle}>{sleepingBadge}{decisionBadge}{presenceBadge}<span className="pointer-events-none absolute left-1/2 top-0 flex h-4 min-w-4 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-warning/40 bg-base-100/90 px-1 text-[0.6rem] font-black leading-none text-warning shadow">{seat.number}</span>{isOwnSeat ? <span className="pointer-events-none absolute -right-3 top-1/4 z-20 -translate-y-1/2 rounded-full border border-primary/50 bg-primary px-0.5 py-0.5 text-[0.55rem] font-black uppercase leading-none text-primary-content shadow sm:-right-4">شما</span> : null}{isLastKing ? <img alt="Last king" className="pointer-events-none absolute z-30 h-7 w-7 rounded-full border border-warning/50 bg-base-100 object-cover p-0.5 shadow-lg sm:h-8 sm:w-8" src="/avalon/King.png" style={markerStyle(2.7)} title={props.lastKingPlayerName ?? `Seat ${seat.number}`} /> : null}{isMateSeat ? <img alt="Selected teammate" className="pointer-events-none absolute z-30 h-7 w-7 rounded-full border border-success/50 bg-base-100 object-cover p-0.5 shadow-lg sm:h-8 sm:w-8" src="/avalon/Mate.png" style={markerStyle(isLastKing ? 3.5 : 2.7)} title={`Seat ${seat.number}`} /> : null}{seat.player ? <><img alt={seatImageAlt} className={`h-9 w-9 rounded-full object-cover transition sm:h-11 sm:w-11 ${isOnline ? "" : "grayscale opacity-55"}`} src={seatImage} /><span className={`pointer-events-none absolute left-1/2 max-w-20 -translate-x-1/2 truncate whitespace-nowrap rounded bg-base-100/85 px-1 text-[0.55rem] font-bold leading-4 text-base-content shadow ${seat.number === 1 ? "bottom-[calc(100%+0.6rem)]" : "top-[calc(100%+0.125rem)]"}`}>{displayName}</span></> : seat.number}</button>;
}
